...
export default function LocationPickerMap({
  ...
  useEffect(() => {
    const loadScript = () => {
      ...
      // Load from env or use fallback Google Cloud API Key
      const apiKey = process.env.NEXT_PUBLIC_GOOGLE_MAPS_KEY || 'AIzaSyA0qPgb21JTzpe0EruEbfLkYrPGSMvM6Co';
  ...
  return (
    <div className="space-y-3 font-sans text-left">
      ...
      <div className="space-y-2">
        <label className="text-[10px] font-mono text-[#5C3D2E] uppercase font-bold block">Pilih dari Lokasi Favorit</label>
        <div className="flex flex-wrap gap-2 max-h-24 overflow-y-auto p-1.5 bg-[#FBF9F6] border border-[#D1C9BC] rounded-xl">
          {presets.length === 0 ? (
            <span className="text-xs text-gray-400 p-1">Belum ada lokasi favorit disimpan.</span>
          ) : (
            presets.map((p, idx) => (
              <button
                key={idx}
                ...
                className={`inline-flex items-center space-x-1 px-2.5 py-1 text-xs rounded-lg transition-colors border ${
                  Math.abs(p.lat - latitude) < 0.0001 && Math.abs(p.lng - longitude) < 0.0001
                    ? 'bg-primary text-[#E7E2D8] border-primary'
                    : 'bg-white text-gray-700 border-[#D1C9BC] hover:bg-slate-100'
                }`}
              >
                ...
                <span
                  ...
                  className="hover:text-red-600 ml-1 font-bold pl-1 border-l border-slate-200 cursor-pointer"
                  title="Hapus"
                >
                  &times;
                </span>
      ...
      {/* The Interactive Map */}
      <div className="w-full h-60 rounded-xl overflow-hidden border border-[#D1C9BC] shadow-sm relative z-0">
      ...
      <div className="flex flex-col sm:flex-row gap-2 items-end sm:items-center justify-between pt-1">
        ...
        <div className="flex items-center space-x-2 w-full sm:w-auto">
          <input
            ...
            className="flex-1 sm:w-40 px-3 py-1 bg-white border border-[#D1C9BC] rounded-lg text-xs focus:outline-none focus:border-primary min-h-[30px]"
          />
          <button
            type="button"
            onClick={saveToPresets}
            className="inline-flex items-center justify-center px-3 py-1 bg-primary text-[#E7E2D8] text-xs font-mono font-bold uppercase rounded-lg hover:bg-opacity-90 min-h-[30px] cursor-pointer"
          >
